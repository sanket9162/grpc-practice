...
func (c *calculatorClient) SendNumber(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[NumberRequest, NumberResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Calculator_ServiceDesc.Streams[1], Calculator_SendNumber_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
func _Calculator_SendNumber_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CalculatorServer).SendNumber(&grpc.GenericServerStream[NumberRequest, NumberResponse]{ServerStream: stream})
}
